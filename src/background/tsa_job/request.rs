...
pub async fn try_tsa_timestamp(
    ...
    // Create TSA client up front: it is needed both to verify a possibly
    // reused anchor below and to request a fresh one further down.
    let client = AsyncRfc3161Client::new()
        .map_err(|e| ServerError::Internal(format!("Failed to create TSA client: {}", e)))?;

    // Check if a TSA anchor already exists for this root_hash.
    let existing = {
        let idx = index.lock().await;
        idx.get_tsa_anchor_with_token_for_hash(&root_hash)
            .map_err(|e| {
    ...
    if let Some(existing) = existing {
        let existing_response = TsaResponse {
            token_der: existing.anchor.token.clone(),
            timestamp: existing.anchor.timestamp,
        };

        if client.verify(&existing_response, &root_hash).is_ok() {
            tracing::info!(
                tree_id = tree.id,
            ...
            let idx = index.lock().await;
            idx.update_tree_tsa_anchor(tree.id, existing.id)
                .map_err(|e| {
                    ServerError::Storage(crate::error::StorageError::Database(e.to_string()))
                })?;
...
mod tests {
    ...
    async fn test_try_tsa_timestamp_existing_anchor() {
        let index = Arc::new(Mutex::new(create_test_index_store()));
        // Reuse requires the stored token to pass messageImprint
        // verification, so the fixture's own embedded hash must be used
        // here (a placeholder token/hash pair would now be discarded and
        // trigger a real network re-request instead of a cheap reuse).
    ...
    async fn test_create_tsa_anchor_for_tree_head_existing_anchor() {
        let index = Arc::new(Mutex::new(create_test_index_store()));
        // See `test_try_tsa_timestamp_existing_anchor`: reuse now requires a
        // verifiable token, so the root hash must match the fixture's own
        // embedded messageImprint.
        let root_hash = freetsa_expected_hash();
    ...
    async fn test_create_tsa_anchor_tree_head_multiple_calls_same_hash() {
        let index = Arc::new(Mutex::new(create_test_index_store()));
        // Reuse now requires a verifiable token; use the fixture's own hash.
        let root_hash = freetsa_expected_hash();
    ...
    /// The bytes a *stored* anchor's `token` column actually holds.
    ///
    /// `freetsa_response_der()` is the raw `TimeStampResp` wire format
    /// (`SEQUENCE { status, timeStampToken }`). What production code stores
    /// (and what `TsaClient::verify` expects) is only the inner
    /// `timeStampToken` (`ContentInfo`), re-encoded by
    /// `parse_timestamp_response` -- see `AsyncRfc3161Client::timestamp`.
    /// Obtain that exact transformation the same way production code does,
    /// via a real (mocked) round-trip, instead of duplicating the ASN.1
    /// extraction logic here.
    async fn freetsa_token_der() -> Vec<u8> {
        ...
        let response = client
            .timestamp(&server.url(), &freetsa_expected_hash(), 5000)
            .await
    ...
    const FREETSA_MISMATCHED_RESPONSE_HEX: &str = "";

    /// Token bytes bound to a DIFFERENT hash than `FREETSA_EXPECTED_HASH_HEX`
    /// (same technique as round_robin.rs: the fixture's 32-byte
    /// `hashedMessage` is byte-swapped, keeping the DER structurally valid
    /// but semantically wrong). Simulates a token stored before
    /// verification-on-receipt existed.
    async fn freetsa_mismatched_token_der() -> Vec<u8> {
        use crate::anchoring::rfc3161::{AsyncRfc3161Client, TsaClient};
        ...
        let _mock = server
            .mock("POST", "/")
            .with_status(200)
            .with_body(hex::decode(FREETSA_MISMATCHED_RESPONSE_HEX).expect("valid hex"))
            .create_async()
            .await;

        let client = AsyncRfc3161Client::new().expect("failed to create TSA client");
        // The mock ignores the request; any hash works to extract the token.
        let response = client
            .timestamp(&server.url(), &[0u8; 32], 5000)
            .await
            .expect("mock TSA round-trip must succeed");
        response.token_der
    }
    ...
    #[tokio::test]
    async fn test_try_tsa_timestamp_discards_bad_existing_anchor_before_reuse() {
        use crate::storage::index::lifecycle::TreeStatus;
        ...
        // Pre-store a "bad" anchor for this hash: a syntactically valid TSA
        // token that is bound to a DIFFERENT hash (exactly the class of
        // record left behind before verification-on-receipt existed).
        ...
        // A working TSA server ready to issue the replacement.
        let mut server = mockito::Server::new_async().await;
        let mock = server
            .mock("POST", "/")
            .with_status(200)
        ...
        assert!(
            result.is_ok(),
            "a bad existing anchor must be replaced, not returned as an error: {:?}",
            result
        );
        ...
        let good_token = freetsa_token_der().await;
        let idx = index.lock().await;

        // The bad row must be gone -- not merely bypassed. In particular it
        // must not appear when a receipt for this tree_size is assembled.
        let anchors_for_receipt = idx.get_anchors(100).expect("get_anchors failed");
        assert!(
            anchors_for_receipt.iter().all(|a| a.token != bad_token),
            "the mismatched token must never be served in a receipt"
        );
        assert!(
            anchors_for_receipt.iter().any(|a| a.token == good_token),
            "the replacement token should be the one served"
        );
